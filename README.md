...
- 🗺️ **滿版互動地圖 (Map-First)**：以台灣為中心的全螢幕 Web GIS 地圖，直觀瀏覽 22 縣市即時氣候與空氣品質徽章。
- 🎨 **玻璃擬物化 (Glassmorphism) 美學**：全域半透明毛玻璃面板（`backdrop-filter: blur`）與微光光暈，打造頂級科技感與空氣感視覺。
- 🍃 **空氣品質 (AQI) 即時圖層**：
  - 整合 Open-Meteo 全台 22 縣市空氣品質與細懸浮微粒 (PM2.5) 觀測數據。
  - 依台灣官方標準呈現 6 級動態色碼徽章（良好 🟢、普通 🟡、對敏感族群不健康 🟠、對所有族群不健康 🔴、非常不健康 🟣、危害 🟤）。
  - 右下角提供浮動式毛玻璃 AQI 指標等級圖例 (Legend)。
- 🔄 **雙資料圖層自由切換**：右上角選單可一鍵快速切換「全台即時天氣」與「空氣品質 (AQI)」圖層。
- 🤖 **定時自動化爬蟲與雲端快取 (Phase 4)**：
  - 由 **GitHub Actions** 設定排程工作流 (`.github/workflows/data-sync.yml`) 每小時自動執行爬蟲腳本。
  - 將氣象署與空氣品質最新資料清洗並寫入 **Supabase** 資料庫快取 (`latest_weather`, `latest_aqi`)。
  - 前端優先秒讀資料庫快取，大幅降低前端 API Key 暴露與限流風險，並保留自動無痛回退機制。
- 🧭 **多樣底圖風格熱切換**：
...
- 💡 **全方位生活與健康出行指南**：結合降雨機率、溫差變化與 AQI 空氣品質健康指數，動態給予綜合生活出行與防疫保護建議。
